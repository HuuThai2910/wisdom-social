...
type PhoneSignupPayload = SignupPayload;
...
const normalizeGender = (gender?: string | null): User["gender"] | undefined => {
    if (!gender) return undefined;
    if (gender === "MALE" || gender === "FEMALE" || gender === "HIDDEN") return gender;
    return undefined;
};
...
const mapApiUserToAppUser = (apiUser: ApiAuthUser): User => {
    const normalizedUsername = apiUser.username?.trim().toLowerCase() || `user${apiUser.id}`;
    return {
...
export function AppProvider({ children }: PropsWithChildren) {
    ...
    const upsertMappedUser = useCallback((mappedUser: User) => {
        setUsers((prev) => {
            const normalized = { ...mappedUser, avatar: mappedUser.avatar || mappedUser.avatarUrl };
            return prev.some((u) => u.id === normalized.id)
                ? prev.map((u) => (u.id === normalized.id ? { ...u, ...normalized } : u))
                : [normalized, ...prev];
        });
    ...
    const upsertUsers = useCallback((incomingUsers: User[]) => {
        setUsers((prev) => {
            ...
            incomingUsers.forEach((user) => {
                map.set(user.id, { ...map.get(user.id), ...user, avatar: user.avatar || user.avatarUrl });
            });
    ...
    const syncCurrentUserFromServer = useCallback(async () => {
        ...
        setDeletionPending(Boolean(latestUser.deletionPending));
        setDeletionRemainingDays(latestUser.deletionPending ? latestUser.deletionRemainingDays : undefined);
    }, [upsertMappedUser]);
    ...
    const refreshPosts = useCallback(async () => {
        ...
        upsertUsers(result.posts.map((post) => post.user).filter(Boolean) as User[]);
        const likedIds = result.posts.filter((post) => post.isLiked).map((post) => post.id);
        const savedIds = result.posts.filter((post) => post.isSaved).map((post) => post.id);
    ...
    useEffect(() => {
        const bootstrapSettings = async () => {
            ...
            if (saved?.notifications) setNotificationSettings((prev) => ({ ...prev, ...saved.notifications }));
        };
    ...
    const persistSettings = async (nextThemeMode: ThemeMode, nextNotificationSettings: NotificationSettings) => {
        await saveSettings({ themeMode: nextThemeMode, notifications: nextNotificationSettings });
        if (loggedIn) {
            const info = await getDeviceInfo();
    ...
    const login = async (phone: string, password: string): Promise<AuthResult> => {
        ...
        if (apiResult.success && apiResult.user) {
            ...
            setDeletionPending(Boolean(apiResult.deletionPending));
            setDeletionRemainingDays(apiResult.deletionPending ? apiResult.deletionRemainingDays : undefined);
            setLoadingAuth(false);
            return { success: true, deletionPending: apiResult.deletionPending, deletionRemainingDays: apiResult.deletionRemainingDays };
        }
        setLoadingAuth(false);
        return { success: false, message: apiResult.message ?? "Đăng nhập thất bại.", remainingSeconds: apiResult.remainingSeconds, lockReason: apiResult.lockReason };
    };

    const signup = async (payload: SignupPayload): Promise<AuthResult> => signupWithPhone(payload);

    const signupWithPhone = async (payload: PhoneSignupPayload): Promise<AuthResult> => {
        setLoadingAuth(true);
        const result = await registerWithPhone({ phone: payload.phone.trim(), password: payload.password, confirmPassword: payload.confirmPassword });
        setLoadingAuth(false);
        return result;
    };

    const verifySignupOtp = async (phone: string, otp: string): Promise<AuthResult> => confirmRegisterOtp(phone, otp);
    const resendSignupOtp = async (phone: string): Promise<AuthResult> => resendRegisterOtp(phone);
    const requestPasswordReset = async (phone: string): Promise<AuthResult> => forgotPassword(phone);
    const resetPasswordByOtp = async (payload: ResetPasswordPayload): Promise<AuthResult> => resetPassword(payload);
    ...
    const likePost = async (postId: string) => {
        ...
        setLikedPostIds((prev) => (wasLiked ? prev.filter((id) => id !== postId) : [postId, ...prev]));
        setPosts((prev) => prev.map((post) => (post.id === postId ? { ...post, likes: wasLiked ? Math.max(0, post.likes - 1) : post.likes + 1, isLiked: !wasLiked } : post)));
        try {
            await togglePostReaction(currentUser.id, postId, "LIKE");
        } catch {
            setLikedPostIds((prev) => (!wasLiked ? prev.filter((id) => id !== postId) : [postId, ...prev]));
            setPosts((prev) => prev.map((post) => (post.id === postId ? { ...post, likes: wasLiked ? post.likes + 1 : Math.max(0, post.likes - 1), isLiked: wasLiked } : post)));
        }
    };
    ...
    const savePost = async (postId: string) => {
        ...
        setPosts((prev) => prev.map((post) => (post.id === postId ? { ...post, isSaved: !wasSaved } : post)));
        try {
    ...
    const addComment = async (postId: string, content: string) => {
        ...
        const localComment = { id: `c${Date.now()}`, userId: currentUser.id, user: currentUser, content: trimmed, text: trimmed, createdAt: new Date().toISOString(), likes: 0 };
        setPosts((prev) => prev.map((post) => (post.id === postId ? { ...post, comments: [...post.comments, localComment], commentsCount: (post.commentsCount ?? post.comments.length) + 1 } : post)));
        try {
    ...
    const createPostWithOptions = async (payload: AddPostPayload): Promise<AuthResult> => {
        if (!currentUser) return { success: false, message: "Please log in first." };
        if (!payload.caption.trim()) return { success: false, message: "Caption is required." };
        const imageUrls = payload.imageUrls?.length ? payload.imageUrls : payload.imageUrl?.trim() ? [payload.imageUrl.trim()] : [];
        try {
            ...
            return { success: false, message: error?.response?.data?.message || "Không thể tạo bài viết" };
        }
    };

    const addPost = async (caption: string, imageUrl: string): Promise<AuthResult> => createPostWithOptions({ caption, imageUrl });
    ...
    const updatePostPrivacyLocal = (postId: string, privacy: string) => {
        setPosts((prev) => prev.map((post) => (post.id === postId ? { ...post, privacy: privacy as PrivacyType } : post)));
    };

    const updateProfile = (payload: UpdateProfilePayload) => {
        if (!currentUser) return;
        setUsers((prev) => prev.map((user) => (user.id === currentUser.id ? { ...user, fullName: payload.fullName?.trim() || user.fullName, bio: payload.bio?.trim() || user.bio, website: payload.website?.trim() || user.website } : user)));
    };

    const sendMessage = async (conversationId: string, content: string): Promise<AuthResult> => {
        if (!currentUser) return { success: false, message: "Please log in first." };
        if (!content.trim()) return { success: false, message: "Message is empty." };
        const message: Message = { id: `m${Date.now()}`, conversationId, senderId: currentUser.id, content: content.trim(), createdAt: new Date().toISOString() };
        const sent = await fakeSendMessage(message);
        setMessages((prev) => [...prev, sent]);
        setConversations((prev) => prev.map((conversation) => (conversation.id === conversationId ? { ...conversation, lastMessage: sent.content, updatedAt: sent.createdAt } : conversation)));
        return { success: true };
    };
    ...
    const searchUsersAndPosts = (query: string) => {
        ...
        const foundUsers = users.filter((user) => user.username.toLowerCase().includes(normalized) || user.fullName.toLowerCase().includes(normalized));
        const foundPosts = posts.filter((post) => {
            const author = post.user || getUserById(post.userId);
            return post.caption.toLowerCase().includes(normalized) || Boolean(author?.username.toLowerCase().includes(normalized));
        });
    ...
    const value = useMemo<AppContextValue>(
        ...
        [users, currentUser, loggedIn, loadingAuth, bootstrapLoading, posts, stories, savedPostIds, likedPostIds, notifications, conversations, messages, igtvVideos, themeMode, notificationSettings, deletionPending, deletionRemainingDays, clearDeletionPending, refreshPosts, getUserById, syncCurrentUserFromServer],
    );
