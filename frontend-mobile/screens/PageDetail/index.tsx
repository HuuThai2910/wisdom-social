...
const buildS3Url = (url?: string): string | undefined => {
...
export default function PageDetailScreen() {
    ...
    const [interaction, setInteraction] = useState<PageInteractionStatus>({
        isLiked: false, isFollowing: false, likeCount: 0, followCount: 0,
    });
    ...
    const isOwner = !!(page && numericUserId && Number(page.createdBy?.id) === numericUserId);
    ...
    useEffect(() => { if (wsRefresh > 0) void load(); }, [wsRefresh, load]);
    ...
    const handleAddMembers = async () => {
        ...
        try {
            await Promise.all(
                selectedUsers.map(u => pageService.addMember(Number(u.id), numericPageId, newMemberRole))
            );
    ...
    const handleDeletePage = () => {
        Alert.alert("Xóa trang", "Bạn có chắc muốn xóa trang này? Hành động không thể hoàn tác.", [
            { text: "Hủy", style: "cancel" },
            {
                text: "Xóa trang", style: "destructive",
                onPress: async () => {
                    setActionLoading(true);
                    try {
    ...
    const renderJoinButton = () => {
        ...
        if (memberStatus === "ACTIVE") {
            return (
                <TouchableOpacity style={[styles.actionBtn, styles.memberBtn]} onPress={handleLeave} disabled={actionLoading}>
        ...
        if (memberStatus === "PENDING") {
            return (
                <TouchableOpacity style={[styles.actionBtn, styles.pendingBtn]} onPress={handleCancelRequest} disabled={actionLoading}>
                    <Ionicons name="time-outline" size={16} color="#F59E0B" />
                    <Text style={[styles.actionBtnText, { color: "#F59E0B" }]}>Hủy yêu cầu</Text>
                </TouchableOpacity>
            );
        }
        return (
        ...
        return (
            <SafeAreaView style={styles.container}>
                ...
                <View style={styles.center}>
                    ...
                    <Text style={{ color: colors.textMuted, marginTop: 10 }}>Không tìm thấy trang</Text>
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            >
                {/* Cover */}
                {page.coverUrl ? (
                    <Image source={{ uri: buildS3Url(page.coverUrl) }} style={styles.cover} resizeMode="cover" />
                ) : (
                ...
                <View style={styles.profileCard}>
                    ...
                    {!!page.description && <Text style={styles.description}>{page.description}</Text>}
                    ...
                    <View style={styles.actionsRow}>
                        ...
                        >
                            ...
                            />
                            <Text style={[styles.actionBtnText, { color: interaction.isLiked ? colors.white : colors.text }]}>
                        ...
                        >
                            <Ionicons
                                name={interaction.isFollowing ? "checkmark-circle" : "add-circle-outline"}
                                size={16}
                                color={interaction.isFollowing ? colors.white : colors.primary}
                            />
                            <Text style={[styles.actionBtnText, { color: interaction.isFollowing ? colors.white : colors.text }]}>
                    ...
                    <View style={{ paddingHorizontal: spacing.lg, paddingTop: spacing.md }}>
                        ...
                        {(canManage || isOwner) && (
                            <View style={styles.card}>
                                ...
                                <ManageBtn icon="add-circle-outline" label="Tạo bài viết" color={colors.primary}
                                    onPress={() => setShowCreatePost(true)} />
                                {(isAdmin || isOwner) && (
                                    <ManageBtn icon="person-add-outline" label="Thêm thành viên" color={colors.primary}
                                        onPress={() => { setMemberQuery(""); setSelectedUsers([]); setShowAddMember(true); }} />
                                )}
                                {(isAdmin || isOwner) && (
                                    <ManageBtn icon="create-outline" label="Chỉnh sửa trang" color={colors.primary}
                                        onPress={() => router.push({ pathname: "/(stack)/page-edit", params: { pageId: String(numericPageId) } })} />
                                )}
                                {isOwner && (
                                    <ManageBtn icon="trash-outline" label="Xóa trang" color={colors.danger}
                                        onPress={handleDeletePage} last />
                                )}
                            </View>
                        )}
                                    ...
                                    pendingRequests.map(req => (
                                        <View key={req.id} style={styles.requestRow}>
                                            <View style={{ flexDirection: "row", alignItems: "center", flex: 1 }}>
                                                {req.user?.avatarUrl ? (
                                                    <Image source={{ uri: buildS3Url(req.user.avatarUrl) }} style={styles.reqAvatar} />
                                                ) : (
                                                    <View style={[styles.reqAvatar, styles.avatarFallback]}>
                                                        <Ionicons name="person" size={14} color={colors.textMuted} />
                                                    </View>
                                                )}
                                                <View style={{ flex: 1, marginLeft: 10 }}>
                                                    <Text style={styles.memberName}>{req.user?.name || req.user?.username || "Người dùng"}</Text>
                                                    <Text style={styles.memberSub}>@{req.user?.username || "unknown"}</Text>
                                                    {req.joinedAt && (
                                                        <Text style={styles.memberSub}>{new Date(req.joinedAt).toLocaleDateString("vi-VN")}</Text>
                                                    )}
                                                </View>
                                            </View>
                                            <View style={{ flexDirection: "row", gap: 8, marginTop: 10 }}>
                                                ...
                                                >
                                                    ...
                                                    <Text style={[styles.smallBtnText, { color: colors.success }]}>Duyệt</Text>
                                                </TouchableOpacity>
                                                <TouchableOpacity
                                                ...
                                                >
                                                    ...
                                                    <Text style={[styles.smallBtnText, { color: colors.danger }]}>Từ chối</Text>
                                                </TouchableOpacity>
                                            </View>
                                        </View>
                                    ))
                        ...
                        {canManage && (postsWaiting.length > 0 || loadingWaiting) && (
                            <View style={styles.card}>
                                <View style={styles.cardHeaderRow}>
                                    <Text style={styles.cardTitle}>Bài viết chờ duyệt ({postsWaiting.length})</Text>
                                ...
                                {postsWaiting.length > 0 && (
                                    <View style={{ flexDirection: "row", gap: 8, marginBottom: 12 }}>
                                        <TouchableOpacity style={[styles.bulkBtn, { backgroundColor: "#D1FAE5" }]} onPress={handleApproveAll}>
                                            <Ionicons name="checkmark-done" size={15} color={colors.success} />
                                            <Text style={[styles.bulkBtnText, { color: colors.success }]}>Duyệt tất cả</Text>
                                        </TouchableOpacity>
                                        <TouchableOpacity style={[styles.bulkBtn, { backgroundColor: "#FEE2E2" }]} onPress={handleCancelAll}>
                                            <Ionicons name="close" size={15} color={colors.danger} />
                                            <Text style={[styles.bulkBtnText, { color: colors.danger }]}>Hủy tất cả</Text>
                                        </TouchableOpacity>
                                    </View>
                                )}
                                {loadingWaiting ? (
                                    ...
                                    postsWaiting.map(post => (
                                        <View key={post.id} style={[styles.postCard, { backgroundColor: "#FEF3C7" }]}>
                                            <View style={styles.postHeader}>
                                                {post.user?.avatarUrl ? (
                                                    <Image source={{ uri: buildS3Url(post.user.avatarUrl) }} style={styles.postAvatar} />
                                                ) : (
                                                ...
                                                <View style={{ flex: 1 }}>
                                                    <Text style={styles.postAuthor}>{post.user?.name || post.user?.username || "Người dùng"}</Text>
                                                    {post.createdAt && (
                                                        <Text style={styles.postDate}>{new Date(post.createdAt).toLocaleDateString("vi-VN")}</Text>
                                                    )}
                                                </View>
                                            ...
                                            {post.images && post.images.length > 0 && (
                                                <Image source={{ uri: buildS3Url(post.images[0]) }} style={styles.postImagePreview} resizeMode="cover" />
                                            )}
                                            <View style={{ flexDirection: "row", gap: 8, marginTop: 10 }}>
                                                <TouchableOpacity style={[styles.smallBtn, { flex: 1, backgroundColor: "#D1FAE5" }]} onPress={() => handleApprovePost(post.id)}>
                                                    <Ionicons name="checkmark" size={16} color={colors.success} />
                                                    <Text style={[styles.smallBtnText, { color: colors.success }]}>Duyệt</Text>
                                                </TouchableOpacity>
                                                <TouchableOpacity style={[styles.smallBtn, { flex: 1, backgroundColor: "#FEE2E2" }]} onPress={() => handleCancelApprovePost(post.id)}>
                                                    <Ionicons name="close" size={16} color={colors.danger} />
                                                    <Text style={[styles.smallBtnText, { color: colors.danger }]}>Hủy</Text>
                                                </TouchableOpacity>
                                            </View>
                                        </View>
                                    ))
                    ...
                    <View style={{ paddingHorizontal: spacing.lg, paddingTop: spacing.md }}>
                        <View style={styles.card}>
                            <View style={styles.cardHeaderRow}>
                                ...
                                {(isAdmin || isOwner) && (
                                    <TouchableOpacity onPress={() => { setMemberQuery(""); setSelectedUsers([]); setShowAddMember(true); }} hitSlop={8}>
                            ...
                            {members.length === 0 ? (
                                ...
                                members.map(m => (
                                    <View key={m.id ?? m.user?.id} style={styles.memberRow}>
                                        {m.user?.avatarUrl ? (
                                            <Image source={{ uri: buildS3Url(m.user.avatarUrl) }} style={styles.memberAvatar} />
                                        ) : (
                                        ...
                                        <View style={{ flex: 1 }}>
                                            <Text style={styles.memberName}>{m.user?.name || m.user?.username || "Người dùng"}</Text>
                                            <View style={{ flexDirection: "row", gap: 6, marginTop: 2 }}>
                                                <View style={[styles.roleBadge, m.role === "ADMIN" && { backgroundColor: colors.primary + "20" }]}>
                                                    <Text style={[styles.roleBadgeText, m.role === "ADMIN" && { color: colors.primary }]}>{m.role}</Text>
                                                </View>
                                            </View>
                                        </View>
                                        {(isAdmin || isOwner) && m.user?.id !== numericUserId && (
                                            <TouchableOpacity onPress={() => handleRemoveMember(m.user?.id ?? 0, m.user?.name || m.user?.username || "thành viên")} hitSlop={8}>
                                ...
                                posts.map(post => (
                                    <View key={post.id} style={styles.postCard}>
                                        <View style={styles.postHeader}>
                                            ...
                                            <View style={{ flex: 1 }}>
                                                <Text style={styles.postAuthor}>Người dùng #{post.authorId}</Text>
                                                {post.createdAt && (
                                                    <Text style={styles.postDate}>{new Date(post.createdAt).toLocaleDateString("vi-VN")}</Text>
                                                )}
                                            </View>
                                        ...
                                        {post.media && post.media.length > 0 && (
                                            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 8 }}
                                                ...
                                                {post.media.map((item, idx) => item?.url ? (
                                                    <Image key={idx} source={{ uri: buildS3Url(item.url) }}
                                                        style={{ width: POST_IMG_WIDTH, height: 220, borderRadius: 10 }} resizeMode="cover" />
                                                ) : null)}
                                            </ScrollView>
                                        )}
                                        {post.stats && (
                                            <View style={{ flexDirection: "row", marginTop: 8, gap: 16 }}>
                                                <Text style={{ fontSize: 13, color: colors.textMuted }}>❤️ {post.stats.likes ?? 0}</Text>
                                                <Text style={{ fontSize: 13, color: colors.textMuted }}>💬 {post.stats.comments ?? 0}</Text>
                                            </View>
                                        )}
                                    </View>
                                ))
                        ...
                        <View style={[styles.sheet, { paddingBottom: insets.bottom + 16 }]}>
                            ...
                            <View style={styles.sheetHeader}>
                                ...
                                <TouchableOpacity onPress={handleCreatePost} disabled={creatingPost} hitSlop={12}>
                                    <Text style={[styles.sheetSave, creatingPost && { color: colors.textMuted }]}>
                            ...
                            <ScrollView style={{ paddingHorizontal: 20 }} keyboardShouldPersistTaps="handled">
                                <TextInput
                                    ...
                                    multiline
                                />
                                <TouchableOpacity style={styles.imagePickerRow} onPress={pickPostImages}>
                                    <Ionicons name="images-outline" size={22} color={colors.primary} />
                                    <Text style={{ fontSize: 14, fontWeight: "600", color: colors.primary }}>Chọn hình ảnh</Text>
                                </TouchableOpacity>
                ...
                <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
                    <View style={styles.overlay}>
                        <View style={[styles.sheet, { paddingBottom: insets.bottom + 16, maxHeight: "80%" }]}>
                            <View style={styles.dragBar} />
                            <View style={styles.sheetHeader}>
                                <TouchableOpacity onPress={() => { setShowAddMember(false); setMemberQuery(""); setSelectedUsers([]); }} hitSlop={12}>
                                    <Text style={styles.sheetCancel}>Hủy</Text>
                                </TouchableOpacity>
                                ...
                                <TouchableOpacity onPress={handleAddMembers} disabled={selectedUsers.length === 0 || addingMembers} hitSlop={12}>
                                    <Text style={[styles.sheetSave, selectedUsers.length === 0 && { color: colors.textMuted }]}>
                            ...
                            <ScrollView style={{ paddingHorizontal: 20, paddingTop: 12 }} keyboardShouldPersistTaps="handled">
                                <TextInput
                                    ...
                                    autoCapitalize="none"
                                />
                                {memberSearching && <ActivityIndicator size="small" color={colors.primary} style={{ marginTop: 12 }} />}
                                {!memberSearching && memberSearchResults.map(u => {
                                    const selected = selectedUsers.some(s => s.id === u.id);
                                    return (
                                        <TouchableOpacity key={u.id} style={[styles.userRow, selected && { borderColor: colors.primary }]}
                                        ...
                                        >
                                            ...
                                            {u.avatarUrl ? (
                                                <Image source={{ uri: buildS3Url(u.avatarUrl) }} style={styles.userAvatar} />
                                            ) : (
                                            ...
                                            <View style={{ flex: 1 }}>
                                                <Text style={styles.memberName}>{u.name || u.fullName}</Text>
                                ...
                                {!memberSearching && memberQuery.length >= 2 && memberSearchResults.length === 0 && (
                                    <Text style={{ textAlign: "center", color: colors.textMuted, marginTop: 20 }}>Không tìm thấy người dùng</Text>
                                )}
                                        ...
                                        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
                                            {selectedUsers.map(u => (
                                                <View key={u.id} style={styles.selectedChip}>
                                                    <Text style={{ fontSize: 13, color: colors.primary, fontWeight: "500" }}>{u.username}</Text>
                                ...
                                <View style={{ flexDirection: "row", gap: 8, flexWrap: "wrap" }}>
                                    {MEMBER_ROLES.map(r => (
                                        <TouchableOpacity key={r.value}
                                            style={[styles.roleChip, newMemberRole === r.value && { backgroundColor: colors.primary, borderColor: colors.primary }]}
                                            onPress={() => setNewMemberRole(r.value)}
                                            activeOpacity={0.7}
                                        >
                                            <Text style={[styles.roleChipText, newMemberRole === r.value && { color: colors.white }]}>{r.label}</Text>
                                        </TouchableOpacity>
                                    ))}
                                </View>
                                <View style={{ height: 20 }} />
...
function ManageBtn({ icon, label, color, onPress, last }: {
    icon: string; label: string; color: string; onPress: () => void; last?: boolean;
}) {
    return (
        <TouchableOpacity style={[{ flexDirection: "row", alignItems: "center", gap: spacing.sm, paddingVertical: 12 },
...
const styles = StyleSheet.create({
    ...
    avatarWrap: { borderWidth: 3, borderColor: colors.white, borderRadius: 44, alignSelf: "flex-start", overflow: "hidden" },
    avatar: { width: 80, height: 80, borderRadius: 40 },
    avatarFallback: { backgroundColor: colors.surface, justifyContent: "center", alignItems: "center" },
    ...
    categoryChip: {
        alignSelf: "flex-start", marginTop: 4, paddingHorizontal: 10, paddingVertical: 3,
    ...
    statsRow: {
        flexDirection: "row", marginTop: spacing.md, paddingVertical: spacing.md,
    ...
    actionsRow: { flexDirection: "row", marginTop: spacing.md, gap: spacing.sm, flexWrap: "wrap" },
    actionBtn: {
        flexDirection: "row", alignItems: "center", gap: 6,
        paddingHorizontal: 14, paddingVertical: 9, borderRadius: 8, borderWidth: 1, borderColor: colors.border,
    },
    ...
    tabBar: { flexDirection: "row", borderTopWidth: 1, borderBottomWidth: 1, borderColor: colors.border },
    ...
    requestRow: { paddingVertical: 10, borderBottomWidth: 1, borderColor: colors.border },
    ...
    postImagePreview: { width: "100%", height: 160, borderRadius: 10, marginTop: 8 },
    ...
    dragBar: { width: 40, height: 4, borderRadius: 2, backgroundColor: colors.border, alignSelf: "center", marginBottom: 8 },
    sheetHeader: {
    ...
    postInput: {
        ...
        fontSize: 15, color: colors.text, height: 120, textAlignVertical: "top", marginTop: 14,
    },
    imagePickerRow: {
        flexDirection: "row", alignItems: "center", justifyContent: "center",
        gap: 8, marginTop: 14, paddingVertical: 14, borderRadius: 12,
        backgroundColor: colors.surface, borderWidth: 1.5, borderStyle: "dashed", borderColor: colors.border,
    },
    removeImageBtn: {
        position: "absolute", top: 8, right: 8,
        backgroundColor: "rgba(0,0,0,0.55)", borderRadius: 20, padding: 5,
    },
    ...
    selectedChip: {
        ...
        paddingLeft: 10, paddingRight: 4, paddingVertical: 6,
        backgroundColor: colors.primary + "20", borderRadius: 20, borderWidth: 1, borderColor: colors.primary,
    },
    roleChip: {
