...
public class UserController {
    ...
    /**
     * [TEST] Đăng nhập bằng username lấy từ DB — không cần password
     * GET /api/auth/test-login?username=john_doe
     */
    @GetMapping("/test-login")
    @ApiMessage("Test login success")
    public ResponseEntity<UserResponseLogin> testLogin(@RequestParam String username) {
        UserResponseLogin response = userService.testLoginByUsername(username);
        if (response == null) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND,
                    "Không tìm thấy user với username: " + username);
        }
