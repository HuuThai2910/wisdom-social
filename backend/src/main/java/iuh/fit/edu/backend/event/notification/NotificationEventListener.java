    ...
    public void handleNotificationEvent(NotificationEvent event) {
        ...
        try {
            ...
            if (event.getRootTargetId() != null) {
                ...
                if (event.getTargetType() == iuh.fit.edu.backend.constant.TargetType.COMMENT) {
                    extraData = "{\"commentId\": \"" + event.getTargetId() + "\"}";
                }
            } else if (event.getType() == iuh.fit.edu.backend.constant.NotificationType.FRIEND_REQUEST || 
                       event.getType() == iuh.fit.edu.backend.constant.NotificationType.FRIEND_ACCEPT) {
