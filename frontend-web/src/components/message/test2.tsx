...
export default function ChatWindow({
    ...
    // Thực thi scroll sau khi DOM đã render xong messages (ổn định hơn requestAnimationFrame trong handler)
    useLayoutEffect(() => {
        const behavior = scrollOnNextRenderRef.current;
        if (!behavior) return;

        // Khi prepend tin nhắn cũ (loadingMore) sẽ tự giữ scrollTop theo delta; không auto-scroll xuống cuối.
        ...
        requestAnimationFrame(() => {
            ...
            // Một số trường hợp layout (ảnh/avatar load chậm) có thể làm lệch vị trí.
            // Re-apply 1 lần (auto) để đảm bảo luôn nằm đúng cuối.
            if (behavior === "auto") {
                setTimeout(() => {
                    scrollToBottom("auto");
                    wasNearBottomRef.current = true;
    ...
    const handleNewMessage = (newMessage: Message) => {
        // Quyết định auto-scroll dựa trên trạng thái DOM hiện tại (tránh stale ref)
    ...
    const loadMessages = async (token: number) => {
        try {
            ...
            // Mở hội thoại -> luôn hiển thị tin nhắn mới nhất
     ...
     */
    const loadMoreMessages = async (options?: { keepAtBottom?: boolean }) => {
        ...
        try {
            ...
            // Auto-fill (khi trang đầu chưa đủ để scroll): luôn giữ user ở cuối để thấy tin mới nhất.
            if (keepAtBottom) {
                scrollOnNextRenderRef.current = "auto";
                setShowScrollToBottomButton(false);
                setPendingNewMessages(0);
