...
function renderHighlightedContent(content: string, keyword: string): ReactNode {
    ...
    while (matchIndex >= 0) {
        ...
        chunks.push(
            <mark
                key={`${matchIndex}-${chunks.length}`}
                className="rounded bg-transparent font-semibold text-blue-600 dark:text-blue-300"
            >
                {content.slice(
                    matchIndex,
                            ...
                            <div className="absolute right-0 top-10 z-50 w-72 rounded-xl border border-gray-200 bg-white p-3 shadow-xl dark:border-gray-700 dark:bg-gray-900">
                                <div className="grid grid-cols-1 gap-1 border-b border-gray-100 pb-2 text-sm dark:border-gray-800">
                                    <button
                                        ...
                                        className="rounded-lg px-2 py-2 text-left text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-800"
                                    >
                                        7 ngày trước
                                    </button>
                                    <button
                                ...
                                {invalidDateRange && (
                                    <p className="mt-2 text-xs font-medium text-red-500">
                                        Ngày kết thúc không được trước ngày bắt
                                        đầu.
                                    </p>
                                )}
                            </div>
                ...
                <div className="flex items-center justify-between rounded-lg bg-gray-100 px-3 py-2 dark:bg-gray-900">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <button
                            ...
                            disabled={
                                loading ||
                                (currentIndex >= results.length - 1 && !hasMore)
                            }
                            className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-gray-600 hover:bg-gray-100 disabled:opacity-40 dark:text-gray-300 dark:hover:bg-gray-800"
...
function ChatWindowContent({
    ...
    const [internalOpenPollMessageId, setInternalOpenPollMessageId] = useState<
        string | null
    >(null);
    const [internalOpenPollModalToken, setInternalOpenPollModalToken] =
        useState(0);
    ...
    const [mediaViewerIndex, setMediaViewerIndex] = useState<number | null>(
        null,
    );
    const [mediaViewerImages, setMediaViewerImages] = useState<
        MediaViewerImage[]
    >([]);
    const [mediaViewerCursor, setMediaViewerCursor] = useState<string | null>(
        null,
    );
    ...
    const loadMoreViewerImages = useCallback(
        async (cursor: string | null = mediaViewerCursor) => {
            ...
            try {
                ...
                setMediaViewerImages((prev) =>
                    mergeMediaViewerImages(prev, incoming),
                );
    ...
    const openMediaViewer = useCallback(
        (url: string) => {
            const initialImages = mergeMediaViewerImages(conversationImages, [
                { url },
            ]);
    ...
    const directTargetUserId = useMemo(() => {
        ...
        if (Number.isFinite(fromMember) && fromMember !== userId)
            return fromMember;
    ...
    const isFriendInContext = useMemo(
        () =>
            contextFriends.some(
                (friend: any) =>
                    String(friend.id) === String(otherMember?.userId),
            ),
    ...
    const isSentRequestInContext = useMemo(
        () =>
            contextSentRequests.some(
                (request: any) =>
                    String(request.id) === String(otherMember?.userId),
            ),
    ...
    const headerStatusText =
        conversation?.type === "DIRECT"
            ? otherMemberOnline
                ? "Đang hoạt động"
                : otherMemberLastActiveText ||
                  resolvedRelationshipText ||
                  "Không hoạt động"
            : null;
                ...
                .map((member) => {
                    ...
                    return realtimeMember
                        ? {
                              ...member,
                              accountLocked:
                                  realtimeMember.accountLocked ??
                                  member.accountLocked,
                              role: realtimeMember.role ?? member.role,
                              nickname:
                                  realtimeMember.nickname || member.nickname,
                              avatar: realtimeMember.avatar || member.avatar,
                              username:
                                  realtimeMember.username || member.username,
                          }
                        : member;
                });
        }
    ...
    const isGroupConversation =
        String(conversationType ?? conversation?.type ?? "").toUpperCase() ===
            "GROUP" || callMemberSource.length > 2;
    ...
    const [pendingIncomingCall, setPendingIncomingCall] =
        useState<CallSignalPayload | null>(() => {
            ...
            try {
                ...
                return parsed?.conversationId === conversationId
                    ? parsed
                    : null;
            } catch {
                return null;
    ...
    const [callPickerMode, setCallPickerMode] = useState<"start" | "invite">(
        "start",
    );
    const [callPickerType, setCallPickerType] = useState<"audio" | "video">(
        "audio",
    );
    ...
    const showAlreadyInCallError = useCallback(() => {
        toast.error(
            "Bạn đang trong một cuộc gọi khác. Hãy kết thúc cuộc gọi hiện tại trước.",
        );
    }, []);
    ...
    const hasInviteCallCandidates = useMemo(() => {
        const activeRemoteIds = new Set(activeCall?.remoteUserIds ?? []);
        return callableMembers.some(
            (member) => !activeRemoteIds.has(member.userId),
        );
    ...
    const openForwardModal = useCallback(
        async (message: Message) => {
            ...
            try {
                const response =
                    await chatService.getForwardableConversations();
        ...
        (image: MediaViewerImage) => {
            const sourceMessage =
                ...
                messages.find((message) =>
                    getMessageImageUrls(message).includes(image.url),
                );
            if (sourceMessage) {
    ...
    const activeOpenPollMessageId =
        openPollMessageId ?? internalOpenPollMessageId;
    ...
    useEffect(() => {
        ...
        activeOpenPollMessageId,
        activeOpenPollModalToken,
        requestJumpToMessage,
    ]);
    ...
    const messageItems = useMemo(() => {
        ...
        const isSystemMessageType = (type?: string) =>
        ...
        for (let idx = 0; idx < messages.length; idx++) {
            ...
            if (isStartOfSystemSequence) {
                ...
                while (
                    ...
                    getDayKey(new Date(messages[groupEnd + 1].createdAt)) ===
                        dayKey
                ) {
                    groupEnd++;
                }
                ...
                if (
                    groupCount >= 3 &&
                    !expandedSystemGroups.has(groupFirstMsgId)
                ) {
                    items.push(
                        <div
                        ...
                        >
                            <button
                                type="button"
                                onClick={() =>
                                    toggleSystemGroup(groupFirstMsgId)
                                }
            ...
            items.push(
                ...
                >
                    <MessageBubble
                        ...
                        openPollModalToken={
                            activeOpenPollMessageId === message.id
                                ? activeOpenPollModalToken
                                : undefined
                        }
                        onPollModalClose={() => {
            ...
            >
              ...
              Tham gia lại
                                ...
                                <div className="max-h-[55vh] overflow-y-auto px-2 py-2">
                                    {forwardLoading ? (
                                        ...
                                        forwardConversations.map((item) => {
                                            const selected =
                                                forwardSelectedIds.has(item.id);
                                            const displayInfo =
                                    ...
                                    <div className="max-h-[55vh] overflow-y-auto px-2 py-2">
                                        {callPickerMembers.length === 0 ? (
                                            <div className="px-3 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                                                Không còn thành viên phù hợp để
                                                gọi.
                                            </div>
                                        ) : (
                                            callPickerMembers.map((member) => {
                                                const checked =
                                                    selectedCallMemberIds.has(
                                                        member.userId,
                                                    );
                                                const disabled =
                                                    !checked &&
                    ...
                    <CallScreen
                        ...
                        remoteAvatar={
                            activeCall?.remoteAvatar || otherMember?.avatar
                        }
